...
class Equiformer_MD17_DeNS(torch.nn.Module):
    def __init__(self,
        ...
        self.backprop = backprop and not onlyEnergy
    ...
    def forward(self, data):
        ...
        if self.backprop and not self.onlyEnergy:
            forces_outputs = -1 * (
                torch.autograd.grad(
                    energy_outputs,
                    pos,
            ...
            forces_outputs = torch.zeros_like(pos)
        ...
        if hasattr(data, 'noise_mask'):
            ...
            if self.backprop:
                return energy_outputs, outputs_dy,0.0
            elif self.onlyEnergy:
                return energy_outputs, outputs_dy,0.0
            else:
                return node_features, outputs_dy,0.0
    
        else:
            if self.backprop:
                return energy_outputs, forces_outputs,0.0
            elif self.onlyEnergy:
                return energy_outputs, forces_outputs,0.0
            else:
                return node_features, forces_outputs,0.0
