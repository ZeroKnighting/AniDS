...
def get_args_parser():
    ...
    parser.add_argument('--freeze_vae', action='store_true', help='freeze vae')


    parser.add_argument("--test_type", type=str, default="AniDS", choices=["AniDS","None"])
    parser.add_argument('--save_ckpt', action='store_true', help='save_ckpt')
    parser.set_defaults(freeze_vae=False)
...
def main(args):
    ...
    model_config["test_type"] = args.test_type
...
def train_one_epoch(args, 
    ...
    loss_metrics = {'energy': AverageMeter(), 'force': AverageMeter(), 'denoising_pos': AverageMeter(), 'kl_loss': AverageMeter()}
    ...
    for step, data in enumerate(data_loader):
        ...
        pred_y, pred_dy,kl_loss = model(data)
        ...
        loss = energy_weight * loss_e + kl_loss * kl_weight
        if not loss_f.isnan():
        ...
        loss_metrics['kl_loss'].update(kl_loss, n=1)
        ...
        if step % print_freq == 0 or step == len(data_loader) - 1: 
            ...
            info_str +=  'loss_e: {loss_e:.5f}, loss_f: {loss_f:.5f}, loss_denoising_pos: {loss_denoising_pos:.5f}, e_MAE: {e_mae:.5f}, f_MAE: {f_mae:.5f}, denoising_pos_MAE: {denoising_pos_mae:.5f}, kl_loss: {kl_loss:.5f}, '.format(
                ...
                kl_loss=loss_metrics['kl_loss'].avg
            )
...
def add_masked_gaussian_noise_to_pos(data, std, prob, corrupt_ratio=None,use_vae=False):
    '''
    ...
    if use_vae is False:
        ...
        data["std"] = std
        data["prob"] = prob
        data["corrupt_ratio"] = corrupt_ratio
