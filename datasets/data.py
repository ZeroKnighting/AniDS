...
class DataModule(LightningDataModule):
    def __init__(self, hparams, dataset=None):
        ...
        self.dict = hparams.__dict__ if hasattr(hparams, "__dict__") else hparams
    ...
    def setup(self, stage):
        if self.dataset is None:
            if self.dict["dataset"] == "Custom":
                self.dataset = pyg.Custom(
                    self.dict["coord_files"],
                    self.dict["embed_files"],
                    self.dict["energy_files"],
                    self.dict["force_files"],
                )
            else:
                if self.dict['position_noise_scale'] > 0.:
                    def transform(data):
                        noise = torch.randn_like(data.pos) * self.dict['position_noise_scale']
                ...
                dataset_factory = lambda t: getattr(pyg, self.dict["dataset"])(self.dict["dataset_root"], dataset_arg=self.dict["dataset_arg"], transform=t)

                # Noisy version of dataset
                # self.dataset_maybe_noisy = dataset_factory(transform)
                self.dataset_maybe_noisy = dataset_factory(None)
                # Clean version of dataset
                self.dataset = dataset_factory(None)

        self.idx_train, self.idx_val, self.idx_test = make_splits(
            len(self.dataset),
            self.dict["train_size"],
            self.dict["val_size"],
            self.dict["test_size"],
            self.dict["seed"],
            join(self.dict["log_dir"], "splits.npz"),
            self.dict["splits"],
        )
        ...
        # self.idx_val = self.idx_train[:20000]
        # self.idx_train=self.idx_train[:1]
    ...
    def val_dataloader(self):
        ...
        if (
            ...
            and self.trainer.current_epoch % self.dict["test_interval"] == 0
        ):
    ...
    def _get_dataloader(self, dataset, stage, store_dataloader=True):
        ...
        if stage == "train":
            ...
            batch_size = self.dict["inference_batch_size"]
            shuffle = False
        ...
        dl = DataLoader(
            ...
            shuffle=shuffle,
            num_workers=self.dict["num_workers"],
            pin_memory=True,
        )
    ...
    def _standardize(self):
        ...
        try:
            ...
            atomref = self.atomref if self.dict["prior_model"] == "Atomref" else None
