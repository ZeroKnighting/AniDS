...
from datasets import pyg
from datasets.data import DataModule
from datasets.utils import LoadFromFile, LoadFromCheckpoint, save_argparse, number
from pathlib import Path
import wandb
from modules import EqV2_AniDS
...
def get_args():
    ...
    # dataset specific
    parser.add_argument('--dataset', default=None, type=str, choices=pyg.__all__, help='Name of the torch_geometric dataset')
    ...
    # model structure
    parser.add_argument('--name', type=str, default='equiformer_md17_dens_vae', help='Name of the model configuration')

    parser.add_argument('--irreps-in', type=str, default='64x0e', help='Input irreducible representations')
    parser.add_argument('--irreps-equivariant-inputs', type=str, default='1x0e+1x1e+1x2e', help='Equivariant inputs for denoising positions')
    parser.add_argument('--irreps-node-embedding', type=str, default='128x0e+64x1e+32x2e', help='Node embedding irreducible representations')
    parser.add_argument('--num-layers', type=int, default=6, help='Number of layers in the model')
    parser.add_argument('--irreps-node-attr', type=str, default='1x0e', help='Node attribute irreducible representations')
    parser.add_argument('--irreps-sh', type=str, default='1x0e+1x1e+1x2e', help='Spherical harmonics irreducible representations')

    parser.add_argument('--max-radius', type=float, default=5.0, help='Maximum radius for neighborhood')
    parser.add_argument('--number-of-basis', type=int, default=32, help='Number of basis functions')
    parser.add_argument('--basis-type', type=str, default='exp', help='Type of basis functions')
    parser.add_argument('--fc-neurons', type=int, nargs='+', default=[64, 64], help='Number of neurons in fully connected layers')

    parser.add_argument('--irreps-feature', type=str, default='512x0e+256x1e+128x2e', help='Feature irreducible representations')
    parser.add_argument('--irreps-head', type=str, default='32x0e+16x1e+8x2e', help='Head irreducible representations')
    parser.add_argument('--num-heads', type=int, default=4, help='Number of attention heads')
    parser.add_argument('--irreps-pre-attn', type=str, default='128x0e+64x1e+32x2e', help='Pre-attention irreducible representations')
    parser.add_argument('--rescale-degree', type=bool, default=False, help='Whether to rescale degree')
    parser.add_argument('--nonlinear-message', type=bool, default=True, help='Whether to use nonlinear message passing')
    parser.add_argument('--irreps-mlp-mid', type=str, default='384x0e+192x1e+96x2e', help='Mid-layer MLP irreducible representations')
    parser.add_argument('--norm-layer', type=str, default='layer', help='Normalization layer type')

    parser.add_argument('--alpha-drop', type=float, default=0.0, help='Dropout rate for alpha')
    parser.add_argument('--proj-drop', type=float, default=0.0, help='Dropout rate after projection')
    parser.add_argument('--out-drop', type=float, default=0.0, help='Dropout rate for output layer')
    parser.add_argument('--drop-path-rate', type=float, default=0.0, help='Drop path rate for stochastic depth')

    parser.add_argument('--use-force-encoding', type=bool, default=True, help='Whether to use force encoding during denoising')
    
    parser.add_argument("--test_type", type=str, default="AniDS", choices=["AniDS"])
...
def main():
    ...
    model = EqV2_AniDS(args, mean=data.mean, std=data.std,test_type=args.test_type)
